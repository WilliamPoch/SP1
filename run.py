...
def main():
    ...
    detector = Detector("ssd_mobilenet_v1_coco_2018_01_28/frozen_inference_graph.pb")
    
    # streamer = FileVideoStream(stream).start()
    streamer = WebcamVideoStream("input/lib2.mp4").start()
    # streamer =  RTSPVideoFeed(stream)
    # streamer.open()

    file_name = 'output/' + str(date.day)  + '-' + str(date.hour) + '.avi'
    ...
    while True:
        ...
        try: 
            ...
            if ret:
                ...
                tracker.update(points, time=ToD, update_type='distance')
                tracker.check()
                ...
                if (end - start == 1):
                    if ToD == "Morning":
                        row = [str(data['date']), str(data['ToD']), str(data['morning']), str(data['in'])]
                        print('Updating...', row)
                        update_csv(row)        
                    else:
                        row = [str(data['date']), str(data['ToD']), str(data['afternoon']), str(data['in'])]
                        print('Updating...', row)
                        update_csv(row)    
                    start = datetime.datetime.now().minute
                    upload_to_cloud("output/peoplecount.csv")
